    ...
    void removeFixup(RBTNode<Key, E> *x)
    {
        ...
        while (x != this->root and !x->isRed())
        {
            if (x == x->parent()->left())
            {
                ...
                // Case 1
                if (w->isRed()) // x's sibling w is red
                {
                    w->makeBlack();
                    x->parent()->makeRed();
                    leftRotate(x->parent());
                    w = x->parent()->right();
                    // Converts case 1(w red) to case 2(w black)
                }
                ...
                if (!(w->left())->isRed() and !(w->right())->isRed())
                {
                    // Case 2.1
                    w->makeRed();
                    x = x->parent(); // making x.p black and fixing
                }
                else
                {
                    if (!(w->right())->isRed())
                    {
                        // Case 2.2
                        // swap w and w.left color
                        (w->left())->makeBlack();
                        w->makeRed();
                        rightRotate(w);
                        w = x->parent()->right();
                        // Converts case 2.2 to case 2.3
                    }

                    // Case 2.3
                    // swap w and x.p color
                    w->setColor(x->parent()->getColor());
                    x->parent()->setColor(BLACK);

                    (w->right())->setColor(BLACK); // send extra black of x to w.right
                    leftRotate(x->parent());

                    // Now we have solved the problem of double black. We can simply terminate.
                    x = this->root;
                }
            }
            else
        ...
        // In case root became red. Make it black. No property violates.
        x->setColor(BLACK);
    }
